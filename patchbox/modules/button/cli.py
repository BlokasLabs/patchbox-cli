...
class PisoundButton(object):
    ...
    def __init__(self, path=None):
        self.path = path or penviron.get(
            'PISOUND_BTN_CFG', debug=False) or settings.BTN_CFG
    ...
    def get_actions(self):
        actions = []
        actions = actions + [{'title': f.split('/')[-1].split('.')[0].replace('_', ' ').title(
        ) + ' ({}.module)'.format(self.module_dir.split('/')[-3]), 'value': f} for f in self._get_module_actions()]
        actions = actions + [{'title': f.split('/')[-1].split('.')[0].replace(
            '_', ' ').title(), 'value': f} for f in self._get_system_actions()]
        return actions
    ...
    def ensure_config(self, interactions=INTERACTIONS):
        ...
        if not isfile(self.path):
            with open(self.path, 'w') as f:
                for key in keys:
                    f.writelines(key + '\t' + self.system_dir +
                                 '/do_nothing.sh' '\n')
        ...
        if len(missing_keys) > 0:
            for key in missing_keys:
                if key == 'CLICK_COUNT_LIMIT':
                    ...
                    continue
                lines.append(
                    str(key + '\t' + self.system_dir + '/do_nothing.sh' '\n'))
    ...
    def get_status(self):
        ...
        if version:
            ...
            for option in self.get_config():
                status += '\n' + \
                    option.get('key', 'UNKNOWN') + '=' + \
                    option.get('path', 'unknown')
        return status
